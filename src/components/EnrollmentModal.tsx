...
const EnrollmentModal: React.FC<EnrollmentModalProps> = ({ isOpen, onClose, onComplete }) => {
  ...
  const processEnrollment = async (samples: Float32Array[]) => {
    ...
    try {
      ...
      const userId = await invoke<number>("biometrics_enroll_user", {
        userName: userName,
  ...
  const renderRecordingStep = () => {
    const recordingIndex = getCurrentRecordingIndex();
    const phrase = RECORDING_PHRASES[recordingIndex];
    ...
    return (
      <>
        <DialogHeader>
          <DialogTitle className="text-xl text-gray-100">Voice Enrollment</DialogTitle>
          <DialogDescription className="text-gray-400">
            Step {getCurrentStepNumber()} of 7: Voice Sample {recordingIndex + 1}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="py-6 space-y-6">
          <div className="text-center">
            ...
            <p className="text-lg font-medium text-purple-200 mb-6">"{phrase}"</p>
            ...
            {!isRecording && audioSamples.length < recordingIndex + 1 && (
              <Button
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && !isRecording && !isProcessing && onClose()}>
      <DialogContent className="sm:max-w-[500px] bg-gray-900 border-gray-800">
        {step === "welcome" && renderWelcome()}
        {step === "name" && renderNameInput()}
        {(step === "record1" || step === "record2" || step === "record3") && renderRecordingStep()}
